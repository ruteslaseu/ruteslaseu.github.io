...
for (let i = 0; i < json.rutes.length; i++) {
    ...
    listInnerHTML +=    `<div class='list_element' id='${element.dificultat.replace(' ', '')}' style="background-color: ${getColor(element)}; ${getColor(element)==="#F9A73E"?"color: #2f2c48;":""}">`;
...
for (let i = 0; i < json.rutes.length; i++) {
    ...
    gridInnerHTML +=        `<img src="${element.imatge}" style="width:100%; border: 3px solid ${getColor(element)};">`;
    gridInnerHTML +=    "</a>";
    gridInnerHTML +=    `<div class="name" style="background: ${getColor(element)}; ${getColor(element)==="#F9A73E"?"color: #2f2c48;":""}">${element.nom}</div>`;
...
function clickDifficulty(index, id){
    ...
    if(difficultiesSelected[index] === true){
        document.querySelectorAll(`[id^='${id}']`)[0].style.background = getColorByIndex(index);
        document.querySelectorAll(`[id^='${id}']`)[1].style.background = getColorByIndex(index);
